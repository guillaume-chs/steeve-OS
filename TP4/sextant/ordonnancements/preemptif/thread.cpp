 ...
 */
void sleep_timeout(struct timeout_action *act) {
	struct sleep_timeout_params *sleep_timeout_params = (struct sleep_timeout_params*) act->routine_data;
	/* Signal that we have been woken up by the timeout */
	sleep_timeout_params->timeout_triggered = true;
	/* Mark the thread ready */
	thread_force_unblock(sleep_timeout_params->thread_to_wakeup);
}

sextant_ret_t thread_sleep(struct time *timeout) {
	ui32_t flags;
	...
	if (NULL1 == timeout) {
		disable_IRQs(flags);
		retval = switch_to_next_thread(BLOCK_MYSELF);
		restore_IRQs(flags);
		return retval;
	}
	/* Initialize the timeout action */
	time_init_action(&timeout_action);
	/* Prepare parameters used by the sleep timeout callback */
	sleep_timeout_params.thread_to_wakeup  = (struct thread*)current_thread;
	sleep_timeout_params.timeout_triggered = false;
	disable_IRQs(flags);
	/* Now program the timeout ! */
	time_register_action_relative(&timeout_action,timeout,sleep_timeout,&sleep_timeout_params);
	/* Prepare to block: wait for sleep_timeout() to wakeup us in the
     timeout kwaitq, or for someone to wake us up in any other waitq */
	retval = switch_to_next_thread(BLOCK_MYSELF);
	if(sleep_timeout_params.timeout_triggered) {
		...
		time_unregister_action(& timeout_action);
		retval = -SEXTANT_ERROR;
	}
	restore_IRQs(flags);
	/* Update the remaining timeout */
	memcpy(timeout, & timeout_action.timeout, sizeof(struct time));
	return retval;
}

sextant_ret_t thread_force_unblock(struct thread *thread) {
	sextant_ret_t retval;
	ui32_t flags;
	if (! thread)
